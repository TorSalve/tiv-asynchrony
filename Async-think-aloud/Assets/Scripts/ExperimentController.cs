...
public class ExperimentController : MonoBehaviour
{
    ...
    private bool hasExperimentEnded = false; // Added to track if the experiment has ended
    public float exposureDuration = 180f;

    [Header("Avatars")]
    public GameObject femaleAvatar;
    public GameObject maleAvatar;
    public GameObject femalePrerecordedAvatar;
    public GameObject malePrerecordedAvatar;
    public GameObject syncAvatar; // Changed from private to public
    ...
    void Start()
    {
        ...
        {
            syncAvatar = (genderMatchedAvatar == GenderMatchedAvatar.woman) ? femalePrerecordedAvatar : malePrerecordedAvatar;
        }
        else
        {
    ...
    // Print finger bones for male and female avatars
    Debug.Log("Female Left Hand Fingers:");
    foreach (var finger in femaleLeftHandFingers)
    {
        Debug.Log(GetHierarchyPath(finger));
    }

    Debug.Log("Female Right Hand Fingers:");
    foreach (var finger in femaleRightHandFingers)
    {
        Debug.Log(GetHierarchyPath(finger));
    }

    Debug.Log("Male Left Hand Fingers:");
    foreach (var finger in maleLeftHandFingers)
    {
        Debug.Log(GetHierarchyPath(finger));
    }

    Debug.Log("Male Right Hand Fingers:");
    foreach (var finger in maleRightHandFingers)
    {
        Debug.Log(GetHierarchyPath(finger));
    }
}

// Helper method to get the hierarchy path of a transform
string GetHierarchyPath(Transform transform)
{
    ...
    void Update()
    {
        ...
        if (isAvatarRunning)
        {
            ...
            else
            {
                ...
                isQuestionnaireRunning = false; // Set questionnaire running to false
                DisplayEndMessage(); // Call DisplayEndMessage directly
            }
        }
    }
    ...
    private void DisplayEndMessage()
    {
        ...
        // Ensure startBox stays inactive and mark the experiment as ended
        startBox.SetActive(false);
    ...
    void ApplyDelayedMovements()
{
    if (movementQueue.Count > 0)
    {
        ...
        // Apply delayed finger rotations to the correct active fingers
        for (int i = 0; i < fingerBonesID.Count; i++)
        {
            if (activeLeftHandFingers != null && activeLeftHandFingers[i] != null)
            {
                Debug.Log($"Applying rotation to left finger {i} - Left: {delayedMovement.leftHandFingerRotations[i]}, Bone: {activeLeftHandFingers[i].name}");
                activeLeftHandFingers[i].rotation = delayedMovement.leftHandFingerRotations[i];
            }
            else
            {
                Debug.LogError($"Left finger {i} transform is not set.");
            }

            if (activeRightHandFingers != null && activeRightHandFingers[i] != null)
            {
                Debug.Log($"Applying rotation to right finger {i} - Right: {delayedMovement.rightHandFingerRotations[i]}, Bone: {activeRightHandFingers[i].name}");
            ...
            {
                Debug.LogError($"Right finger {i} transform is not set.");
            }
        }
    }
}
    ...
    {
        femaleAvatar.SetActive(activeAvatar == femaleAvatar);
        maleAvatar.SetActive(activeAvatar == maleAvatar);
        femalePrerecordedAvatar.SetActive(activeAvatar == femalePrerecordedAvatar);
        malePrerecordedAvatar.SetActive(activeAvatar == malePrerecordedAvatar);
    }
